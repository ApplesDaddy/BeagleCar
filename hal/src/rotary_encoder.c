...
static void set_counter_clkwise(void) { clkwise = false; }
static void set_clkwise(void) { clkwise = true; }
static void handle_1_to_0(void) {
    if (!clkwise) {
        // Left turn
        counter -= step;
...
void *rot_encoder_state_thread(void *args) {
    ...
    do {
        ...
        for (int i = 0; i < num_events; i++) {
            ...
            if (is_rising) {
                state_event = line_number == GPIO_LINE_NUMBER_A ? &current_state->rising_a : &current_state->rising_b;
            } else {
                state_event = line_number == GPIO_LINE_NUMBER_A ? &current_state->falling_a : &current_state->falling_b;
            }
...
void rot_encoder_set_step(int count) { step = count; }
